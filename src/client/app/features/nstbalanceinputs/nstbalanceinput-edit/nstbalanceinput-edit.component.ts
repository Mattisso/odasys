...
export class NstbalanceinputEditComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    saveBalance(): void {
            if (this.nstbalanceinputForm.valid) {
          if (this.nstbalanceinputForm.dirty) {

          //    const p = { ...this.balance, ...this.nstbalanceinputForm.value };
              const p = Object.assign({}, this.balance, this.nstbalanceinputForm.value);

              if (p.id === '0') {

                  this.balanceInputService.createBalance(p)
                      .subscribe(
                  ...
                  this.balanceInputService.updateBalance(p)
                      .subscribe(
                          () => this.onSaveComplete(`${this.balance.IntitulCompte} was saved`),
                          (error: any) => this.errorMessage = <any>error
                      );
    ...
    showConfig() {
      this.balanceInputService.getBalanceUrl()
        .subscribe(
          (data: INstbalanceinput) => this.balance = { ...data }, // success path
          error => this.error = error // error path
        );
    }
    showConfigResponse() {
      this.balanceInputService.getBalanceResponse()
